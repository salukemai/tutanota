...
export class SubscriptionSelector implements MComponent<SubscriptionSelectorAttr> {
	...
	_getCurrentPlanInfo(selectorAttrs: SubscriptionSelectorAttr): ?string {
		if (selectorAttrs.options.businessUse() && selectorAttrs.currentSubscriptionType && !selectorAttrs.currentlyBusinessOrdered) {
			...
			return lang.get("businessCustomerNeedsBusinessFeaturePlan_msg", {"{price}": price, "{plan}": selectorAttrs.currentSubscriptionType})
	...
	_createUpgradeBoxAttr(selectorAttrs: SubscriptionSelectorAttr, targetSubscription: SubscriptionTypeEnum): BuyOptionBoxAttr {
		...
		const premiumFeatures = [
			...
			lang.get(targetSubscriptionConfig.business || selectorAttrs.currentlyBusinessOrdered
